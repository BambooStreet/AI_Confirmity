...
import { preSurveyQuestionsEn } from "@/data/survey-questions.en";
...
export default function PreSurveyPage() {
  ...
  useEffect(() => {
    let active = true;
    fetch(`/api/survey?type=pre&lang=${lang}`, { cache: "no-store" })
      ...
      .catch(() => {
        ...
        if (active)
          setQuestions(lang === "en" ? preSurveyQuestionsEn : preSurveyQuestions);
      });
    return () => {
      active = false;
    };
  ...
  return (
    <PageWrapper currentStep="pre-survey">
      <h1 className="text-2xl font-bold text-gray-900 mb-2">{t.preTitle}</h1>
      <p className="text-sm text-gray-500 mb-6">{t.preDesc}</p>
